...
class ReportesViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private suspend fun obtenerDatosLuminarias(inicio: LocalDate, fin: LocalDate): DatosLuminarias {
        ...
        val totalMateriales = reparacionesFiltradas.sumOf { it.cantidadUtilizada }
        val codigosDistintos = reparacionesFiltradas.map { it.codigoMaterial }
            .filter { it.isNotBlank() }
            .distinct()
            .size
        return DatosLuminarias(
            reparaciones = reparacionesFiltradas,
            totalMateriales = totalMateriales,
            codigosDistintos = codigosDistintos,
            existenciasActuales = buildExistenciasActuales(inventario)
        )
    }
    ...
    private suspend fun mapLuminarias(base: DatosLuminarias): List<LuminariaReparadaReportItem> {
        ...
        return base.reparaciones.map { reparacion ->
            ...
            val existencia = buscarExistenciaActual(
                base.existenciasActuales,
                reparacion.codigoMaterial,
                descripcion
            )
