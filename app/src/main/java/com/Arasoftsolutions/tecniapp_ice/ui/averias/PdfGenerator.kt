...
object PdfGenerator {
  ...
  suspend fun exportAveria(context: Context, item: AveriaUI) = withContext(Dispatchers.IO) {
    ...
    try {
      ...
      val inicioAtencion = formatMillis(item.horaInicio)
      val finAtencion = formatMillis(item.horaFinal)
      ...
      drawLine(context.getString(R.string.averia_reporte_materiales, item.materiales.ifBlank { "—" }))
