...
export default function AveriaDetail() {
  ...
  const averia = useMemo(() => averias.find(a => a.caseId === caseId), [averias, caseId])
  ...
  const handleUpdateEstado = async (newEstado: AveriaEstado) => {
    ...
    setSaving(true)
    try {
      await update(ref(rtdbAverias, `averias/${averia.caseId}`), {
        estado: newEstado,
  ...
  const handleAssign = async () => {
    ...
    setSaving(true)
    const tecnico = usuarios.find(u => u.uid === assignTecnico)
    try {
      await update(ref(rtdbAverias, `averias/${averia.caseId}`), {
        estado: 'ASIGNADA' as AveriaEstado,
        vehiculoAsignado: assignVehiculo,
        tecnicoAsignadoUid: assignTecnico,
        tecnicoAsignadoNombre: tecnico ? `${tecnico.nombre} ${tecnico.apellidos}` : assignTecnico,
        lastUpdated: Date.now(),
      })
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-xl font-bold text-slate-800 font-mono">Caso #{averia.caseId}</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          {averia.estado === 'ASIGNADA' && (
            ...
            >
              {saving ? <Spinner size="sm" /> : 'Iniciar Atención'}
            </button>
          )}
      ...
      {isReadOnly && (
        <div className={`rounded-xl p-4 flex items-center gap-3 ${averia.estado === 'RESUELTA' ? 'bg-green-50 border border-green-200' : 'bg-gray-50 border border-gray-200'}`}>
          ...
          <p className={`text-sm font-medium ${averia.estado === 'RESUELTA' ? 'text-green-700' : 'text-gray-600'}`}>
            {averia.estado === 'RESUELTA' ? 'Esta avería ha sido resuelta exitosamente.' : 'Esta avería fue anulada y está en modo solo lectura.'}
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-100 p-5">
            ...
            {averia.lat && averia.lng && (
              <InfoRow label="Coordenadas" value={`${averia.lat.toFixed(6)}, ${averia.lng.toFixed(6)}`} mono />
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          {(averia.kilometrajeInicio > 0 || averia.kilometrajeLlegada > 0) && (
            <div className="bg-white rounded-xl border border-slate-100 p-5">
              ...
              <InfoRow label="Inicio" value={averia.kilometrajeInicio > 0 ? `${averia.kilometrajeInicio.toLocaleString()} km` : null} />
              <InfoRow label="Llegada" value={averia.kilometrajeLlegada > 0 ? `${averia.kilometrajeLlegada.toLocaleString()} km` : null} />
              <InfoRow label="Final" value={averia.kilometrajeFinal > 0 ? `${averia.kilometrajeFinal.toLocaleString()} km` : null} />
              ...
              <div className="space-y-2">
                {tecnicos.map((t: any, i: number) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700">{t.nombre || t.name || JSON.stringify(t)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {evidencias.length > 0 && (
            <div className="bg-white rounded-xl border border-slate-100 p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-base font-semibold text-slate-800">Evidencias</h3>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {evidencias.map((e: any, i: number) => {
                  const url = typeof e === 'string' ? e : e.url || e.uri
                  return url ? (
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  {vehiculos.map(v => (
                    <option key={v.vehiculoId} value={v.placa}>{v.placa} — {v.tipo}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  {usuarios.filter(u => u.rol === 'tecnico').map(u => (
                    <option key={u.uid} value={u.uid}>{u.nombre} {u.apellidos}</option>
                  ))}
                </select>
              </div>
            </div>
