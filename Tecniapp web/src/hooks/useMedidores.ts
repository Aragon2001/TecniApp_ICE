...
export function useMedidores(): UseMedidoresResult {
  ...
  const buscarMedidor = useCallback(async (numero: string) => {
    ...
    // 1. Check Dexie cache first (offline-first)
    try {
      ...
      if (cachedAll.length > 0) {
        ...
        // If cache exists but medidor not found, only report not found if offline
        if (!navigator.onLine) {
    ...
    try {
      ...
      // 2. Search user's subregion first
      if (userSubregion) {
      ...
      // 3. Fallback: search all subregions
